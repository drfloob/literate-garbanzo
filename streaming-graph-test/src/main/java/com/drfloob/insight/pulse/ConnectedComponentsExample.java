...
package com.drfloob.insight.pulse;
...
public class ConnectedComponentsExample implements ProgramDescription {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<DisjointSet<Long>> cc = edges.aggregate(new WindowedConnectedComponents<Long, NullValue>(mergeWindowTime));
        // cc.print();
        ...
        cc
		.timeWindowAll(Time.of(printWindowTime, TimeUnit.MILLISECONDS))
//                .windowAll(SlidingEventTimeWindows.of(
//                        Time.of(printWindowTime, TimeUnit.MILLISECONDS),
//                        Time.of(printWindowTime/2, TimeUnit.MILLISECONDS)))
                .apply(new AllWindowFunction<DisjointSet<Long>, String, TimeWindow>() {
                    @Override
                    public void apply(TimeWindow window, Iterable<DisjointSet<Long>> values, Collector<String> out)
                            throws Exception {
                        for (DisjointSet<Long> djs : values)
    ...
    private static GraphStream<Long, NullValue, NullValue> getGraphStream(StreamExecutionEnvironment env) {
        ...
        if (fileOutput) {
            DataStreamSource<String> src = env.readTextFile(edgeInputPath);
            SingleOutputStreamOperator<String> src2 = src.assignTimestampsAndWatermarks(new IngestionTimeExtractor<String>());
            SimpleEdgeStream<Long, NullValue> ses = new SimpleEdgeStream<Long, NullValue>(
                    src2.map(new MapFunction<String, Edge<Long, NullValue>>() {
                        @Override
